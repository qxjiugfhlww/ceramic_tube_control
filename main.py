    ...
    def __init__(self):
        ...
        def rotate(self, x,y,z,axis_x, axis_y, axis_z, theta):
            ...
            for i in range(len(x)):
                x[i],y[i],z[i] = np.dot(rotation_matrix(self, axis, theta), [x[i],y[i],z[i]])
            return x,y,z
        ...
        def createEllipse(self, x,y,z,rx,ry,rz, index, x_prev=None, y_prev=None, z_prev=None, type=None, color=None):
            ...
            if (index == len(x)-1):
                perp_x1, perp_y1, perp_x2, perp_y2 = getPerpendicular(self, x[index],y[index],x[index-1],y[index-1], 0.3)
                perp_x3, perp_z1, perp_x4, perp_z2 = getPerpendicular(self, x[index],z[index],x[index-1],z[index-1], 0.3)
            ...
            xyz_angles = getXYZAngles(self, perp_x3,y[index],perp_z1,x[index],y[index],z[index],perp_x1,perp_y1,z[index])
            ...
            e_x, e_y, e_z = rotate(self, e_x,e_y,e_z,1, 0, 0, theta)
            if (type == 'skelet'):
                #e_x_1d, e_y_1d, e_z_1d = rotate(e_x_1d, e_y_1d, e_z_1d,1, 0, 0, theta)
                ed_x, ed_y, ed_z = rotate(self, ed_x,ed_y,ed_z,1, 0, 0, theta)
            ...
            e_x, e_y, e_z = rotate(self, e_x,e_y,e_z,0, 1, 0, theta)
            if (type == 'skelet'):
                e_x_1d, e_y_1d, e_z_1d = rotate(self, e_x_1d, e_y_1d, e_z_1d,0, 1, 0, theta)
                ed_x, ed_y, ed_z = rotate(self, ed_x,ed_y,ed_z,0, 1, 0, theta)

            #print("angle1:", angle1, angle1*180/np.pi)
            angleBetweenPointsXY = getAngleBetweenPointsXY(self, x[index],0,0,0)
            theta = xyz_angles[1]#gaz #np.radians(55) #vg_angle_z#angle1#angleBetweenPointsXY
            #print("z:", theta, theta*180/np.pi)
        
            e_x, e_y, e_z = rotate(self, e_x,e_y,e_z,0, 0, 1, theta)
            if (type == 'skelet'):
                e_x_1d, e_y_1d, e_z_1d = rotate(self, e_x_1d, e_y_1d, e_z_1d,0, 0, 1, theta)
                ed_x, ed_y, ed_z = rotate(self, ed_x,ed_y,ed_z,0, 0, 1, theta)
        ...
        def plot1(self): 
            ...
            for i in range(len(x_centers_curve)):
                ...
                c_e = createEllipse(self, x_centers_curve,y_centers_curve,z_centers_curve,0,radiuses_axes_ellipse[tmp_counter][0],radiuses_axes_ellipse[tmp_counter][0], int(i), type='skelet', color='tab:orange')
                if (c_e != None):
                    curve_e_x.append(c_e[0]); curve_e_y.append(c_e[1]); curve_e_z.append(c_e[2])
                s_e = createEllipse(self,x_centers_straight,y_centers_straight,z_centers_straight,0,0.5,0.5, int(i), type='skelet', color='tab:blue')
                if (s_e != None):
